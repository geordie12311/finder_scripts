...
rprint(
    "[bold red on yellow]***************************THIS SCRIPT WILL FIND AN IP ROUTE ON THE NETWORK*************************[/bold red on yellow]"
)
time.sleep(2)
...
def get_cisco_routes(task):
    ...
    for cisco_prefix in prefixes:
        ...
        if ipaddr in network:
            ...
            if source_prot == "connected":
                try:
                    outgoing_intf = prefixes[cisco_prefix]["next_hop"][
                        "outgoing_interface"
                    ]
                ...
                try:
                    ...
                    for key in next_hop_list:
                        ...
                        if route_preference == 20:
                            ...
                            source_prot = "iBGP"
                        my_list.append(
                            f"{task.host} can reach {target} via next hop {next_hop} ({source_prot})"
                        )
                except KeyError:
                    pass
    for uptime in next_hop_list:
        ...
        if route_uptime < "01.00.00":
            my_list.append(
                f"{task.host} next hop {next_hop} route uptime is less than 1hr"
            )
        else:
            my_list.append(
                f"{task.host} next hop {next_hop} route uptime is more than 1hr"
            )
